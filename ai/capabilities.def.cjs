...
const CAPABILITIES = [
  {
    id: 'project.setup', group: 'onboarding',
    title: 'Cài dependency và đồng bộ project từ shared kit đã pin',
    npm: 'npm run setup', cmd: `node ${TOOLS}/setup-project.cjs`, args: [], optional: [],
    when: 'Sau clone/init submodule hoặc khi cần đồng bộ đủ package, extension, launcher và AI skills.',
    outputs: ['package.json, package-lock.json, .npmrc, shared scripts/extensions, BAT/SH launchers, provider skills'],
    limits: ['Không fetch shared-kit mới và không ghi đè scene/settings của game. Cần Node, npm, Git và Cocos đã cài; CLI build cần đóng editor của cùng project sau khi lưu.'],
    status: 'ok', probe: 'help', probeCmd: `node ${TOOLS}/setup-project.cjs`, expect: ['project-root', 'Installs'],
  },
  {
  ...
  },
  {
    ...
    outputs: ['stdout JSON bounded: exact submodule, generated contract, provider skills, Work Memory integrity, full tracked regression dependency closure và local receipt disposition'],
    limits: [
  ...
  // ──────────────────────────────────────────────────────────────── port ──
  {
    ...
    limits: [
      ...
      'Chỉ chẩn đoán UNITY_PACKAGE_TLS_CERTIFICATE_ERROR khi Curl error 35/UnityTls 7 có ngữ cảnh Package Manager trong log của đúng project. TLS chung từ Unity services không chứng minh MCP download thất bại; giữ timeout gốc nếu thiếu attribution. Không suy package cụ thể từ lỗi mạng chung.',
  ...
  },
  {
    ...
    expect: ['--project', '--provider', '--bootstrap', '--intent', '--target', '--profile', '--dispositions', '--cache-dir'],
  },
  {
    id: 'port.core.init',
    group: 'port',
    ...
    optional: ['--entry-scene <Assets/...unity>', '--scaffold-receipt <file>', '--provider <auto|static|unity-mcp>', '--bootstrap', '--dispositions <file>', '--cache-dir <dir>', '--no-cache', '--refresh-cache', '--manifest <file>', '--target-scene <file.scene>', '--force', '--dry-run', '--json'],
    when: 'Bước đầu implementation core playable. Lệnh tự chạy preflight playable-core, khóa gameplay entry/closure, apply exact engineFeatureClosure vào active Cocos preview rồi mới tạo manifest evidence nhỏ.',
    outputs: ['Cocos Feature Cropping profile + active preview apply evidence khi source yêu cầu', '<CocosProjectRoot>/.ai/port/core-gameplay.json', 'stdout JSON compact core scope + engine feature gate + next action'],
    limits: [
    ...
    expect: ['--unity-project', '--cocos-project', '--provider', '--dispositions', '--cache-dir', '--no-cache', '--refresh-cache', '--target-scene', '--dry-run', '--force'],
  },
  {
    ...
    args: ['--unity-project <UnityProjectRoot>', '--cocos-project <CocosProjectRoot>'],
    optional: ['--entry-scene <Assets/...unity>', '--scaffold-receipt <file>', '--provider <static|auto|unity-mcp>', '--bootstrap', '--dispositions <file>', '--cache-dir <dir>', '--no-cache', '--refresh-cache', '--manifest <file>', '--wiring <file>', '--packet <file>', '--target-scene <file.scene>', '--force', '--dry-run', '--json'],
    ...
    expect: ['scaffold', '--unity-project', '--cocos-project', '--provider', '--cache-dir', '--no-cache', '--refresh-cache', '--wiring', '--packet', '--dry-run'],
  },
  {
    id: 'port.core.resume',
    group: 'port',
  ...
  {
    id: 'unity.intel.reference.capture',
    group: 'port',
    title: 'Capture deterministic native Unity frames and particle spatial evidence',
    cmd: `node ${TOOLS}/unity-intel/unity-reference-capture.cjs`,
    args: ['--project <UnityProjectRoot>', '--scene <Assets/...unity>', '--out <dir>', '--frames <n,n,...>'],
    optional: ['--width <n>', '--height <n>', '--frame-rate <n>', '--seed <n>', '--camera <path>', '--spawns <json>', '--disable-component <MonoBehaviourType>', '--field Component.field=value', '--timeout-ms <n>'],
    when: 'After authenticated live Unity setup, before measuring Cocos visual and particle behavior parity.',
    outputs: ['PNG per requested frame', 'manifest.json with camera, viewport, time, counts, depth and in-view bins'],
    limits: [
      'Editor batch mode requires graphics and captures in PostLateUpdate after particle jobs; WaitForEndOfFrame is not invoked in Editor batch mode.',
      'Success requires every unique requested frame, a camera, existing canonical PNG paths and matching viewport dimensions. An empty/partial manifest fails closed even if complete=true.',
      'Refresh the Unity package after changing capture C# and wait for compilation/domain reload before taking new evidence. A running batch Editor does not reliably auto-import external changes.',
      'Use source camera, materials, imported texture resolution and fixed timestep. Capture evidence establishes a reference; it does not establish Cocos parity.',
      'To isolate a spawned source prefab, --disable-component explicitly disables its demo cycler in Play Mode before Start. Missing types fail; source scene and assets are never saved.',
    ],
    status: 'ok',
    probe: 'help',
    probeCmd: `node ${TOOLS}/unity-intel/unity-reference-capture.cjs --help`,
    expect: ['--frames', '--spawns', '--field', '--width'],
  },
  {
    id: 'unity.intel.scan',
    group: 'port',
  ...
  },
  {
    ...
    optional: [
      ...
      '--script-mode <mode>', '--layer-map <json>',
    ...
    limits: [
      ...
      'Unity UI Particle đi qua CanvasRenderer có thể nằm trên HUD theo Canvas sorting, còn Cocos ParticleSystem vẫn render ở camera 3D trước UI batch dù là sibling cuối. Khi particle phải phủ HUD, tách nó sang custom layer và camera orthographic clear depth-only có priority cao hơn UI camera; background phải ở layer riêng để overlay camera không vẽ lại nền.',
      ...
      'Khi quy đổi RectTransform Unity sang local position Cocos, anchoredPosition chỉ được cộng một lần vào điểm anchor của parent: ((anchorMin + stretch * pivot) - parentPivot) * parentSize + anchoredPosition. Không cộng thêm m_LocalPosition hoặc anchoredPosition lần hai. Với nested prefab, lấy size/pivot từ RectTransform parent hiệu dụng sau override và thêm regression có anchor/pivot không ở giữa.',
      ...
      'Unity TextureImporter bỏ qua colour profile nhúng trong PNG (iCCP/gAMA/cHRM/sRGB/cICP) và render texel thô, còn browser và Cocos web (createImageBitmap không truyền colorSpaceConversion:none) lại áp profile nên màu lệch. Mọi đường copy/refresh/resize ảnh Unity đi qua một writer chung: resize dùng sharp ignoreIcc, bản Cocos luôn bỏ các chunk đó (IDAT giữ nguyên byte), rồi áp alphaUsage; so byte trước khi ghi nên port lặp không rewrite hay kích reimport. Report `TEXTURE_COLOR_PROFILE_STRIPPED` khi có chunk bị bỏ. `.meta`/UUID giữ nguyên; bản copy cũ còn profile được refresh khi port lại và phải reimport qua Cocos AssetDB. ICC nhúng trong JPEG/WebP không resize vẫn được copy nguyên byte, là evidence gap cần kiểm màu nếu ảnh có profile.',
  ...
  },
  {
    id: 'font.convert',
    group: 'port',
    title: 'Chuyển font Unity/webfont lạ sang TTF Cocos có thể import',
    npm: null,
    cmd: `node ${TOOLS}/font-converter.cjs`,
    args: ['--src <font>', '--out <assets/.../font.ttf>'],
    optional: ['--json'],
    when: 'Khi font Unity không thuộc định dạng Cocos Creator 3.8.8 nạp trực tiếp. `port.prefab` tự gọi chuỗi này; dùng CLI riêng để chẩn đoán hoặc chuyển một font.',
    outputs: ['<out>.ttf', 'stdout conversion receipt/backend'],
    limits: [
    ...
    status: 'partial',
    verify: 'npm run ai:verify:assets',
    probe: 'help',
    probeCmd: `node ${TOOLS}/font-converter.cjs`,
    expect: ['--src', '--out', '--json'],
  },
  {
    id: 'font.subset',
    group: 'optimize',
    title: 'Subset TTF theo character closure portable và khóa budget 80/100 KiB',
    npm: 'npm run ai:font:subset -- --config tools/font-subsets.json --verify',
    cmd: `node ${TOOLS}/font-subsetter.cjs`,
    args: ['--config <tools/font-subsets.json>'],
    optional: ['--unity-project <UnityProjectRoot>', '--project-root <CocosProjectRoot>', '--write', '--check', '--verify', '--json'],
    when: 'Khi một TTF reachable vượt target 80 KiB hoặc hard limit 100 KiB, sau khi đã khóa font nguồn và character inventory thật.',
    outputs: ['subset TTF ở path khai báo', 'stdout source/output SHA-256, size, glyph coverage và meta-preservation receipt'],
    limits: [
    ...
    verify: 'npm run ai:verify:assets',
    probe: 'help',
    probeCmd: `node ${TOOLS}/font-subsetter.cjs`,
    expect: ['--config', '--unity-project', '--write', '--check', '--verify', '--json'],
  },
  {
    id: 'port.smart',
    group: 'port',
  ...
  },
  {
    id: 'shader.chain',
    group: 'port',
    title: 'Port scene, prefab hoặc ScriptableObject material-set kèm shader + texture phụ thuộc (ENTRY POINT)',
    npm: null,
    cmd: `node ${TOOLS}/shader-compiler/unity-shader-compiler.cjs chain`,
    args: ['--src <file.unity|file.prefab|file.asset>', '--unity-root <UnityAssetsFolder>'],
    optional: ['--out-dir <assets/>', '--json', '--no-cache', '--max-closure-assets <n>', '--max-closure-depth <n>'],
    when: 'DÙNG ĐẦU TIÊN khi cần port scene gameplay, prefab hoặc ScriptableObject chứa material-set active/disabled. Tự đi hết chuỗi scene/asset -> nested prefab -> material -> shader -> texture, transpile luôn, và chỉ in ra phần cần quyết định. Đừng tự đọc YAML của scene/prefab/.asset/.mat.',
  ...
  // ────────────────────────────────────────────────────────────── verify ──
  {
    ...
    optional: ['--no-restart', '--max-mcp-attempts <n>', '--mcp-url <url>', '--physics-backend <name>', '--physics-2d-backend <name>', '--spine-backend <name>', '--force-backend', '--dry-run', '--json'],
    when: 'Core init/scaffold gọi gate từ Unity engineFeatureClosure trước gameplay implementation; sau port tool còn audit component/API Cocos thực dùng. Tool thử Cocos-MCP Profile API tối đa 2 lượt, rồi mới fallback exact-backup + lock + CAS patch engine.json và restart bằng 1_open-project.bat.',
    ...
    expect: ['audit', 'ensure', 'patch', '--project', '--no-restart', '--physics-backend', '--physics-2d-backend', '--spine-backend'],
  },
  {
    id: 'port.core.acceptance',
    group: 'verify',
    title: 'Cổng build hoặc preview + evidence-weighted core gameplay fidelity 80/90',
    ...
    args: ['--unity-project <UnityProjectRoot>', '--cocos-project <CocosProjectRoot>'],
    optional: ['--manifest <file>', '--preview-only', '--preview-url <loopback-origin>', '--no-run-gates', '--json'],
    when: 'Cổng kết thúc port core. Mặc định chạy build acceptance; khi user cấm build, dùng --preview-only để nghiệm thu active Cocos editor preview mà không tạo/đọc build artifact.',
    outputs: ['stdout JSON compact: mode/status, build runnable hoặc preview-runnable, artifact presence, fidelity score/checkpoints, nextActions'],
    limits: [
      'Score là rubric parity có evidence, không phải pixel-perfect hoặc semantic proof tự động; target 90 và minimum accept 80.',
      'User yêu cầu preview-only: dùng --preview-url http://localhost:7456/. Giữ verify/lint/assets/regressions/runtime và evidence rubric; chỉ loại build.playable/builtHtml. Report deliveryMode=browser-preview, packagedReadiness=not-assessed; không tạo build receipt.',
      'Compiler confidence, status boolean hoặc file tồn tại đơn lẻ không được tính điểm. Evidence schema v2 phải bind current brief/state/checkpoint và SHA-256 của từng Cocos target theo contract `sha256-text-lf-v1` (text gộp CRLF->LF, binary png/jpg/mp3/fbx... giữ byte-exact) để evidence commit trên checkout CRLF vẫn khớp checkout LF; tính bằng `currentTargetHashes`, không dùng sha256sum thô. Evidence v1 raw-byte không còn được tính, manifest cũ vẫn hợp lệ; nội dung target đổi làm evidence stale. Mandatory input/rules/win-lose chỉ nhận runtime, không nhận visual-only.',
    ...
    expect: ['--unity-project', '--cocos-project', '--preview-only', '--preview-url', '--no-run-gates', '--manifest'],
  },
  {
    id: 'gameplay.briefs.verify',
    group: 'verify',
    title: 'Kiểm tra selector, asset bundle và build config của mọi playable brief',
    npm: 'npm run briefs:check',
    cmd: `node ${TOOLS}/gameplay-brief-validator.cjs`,
    args: [],
    optional: ['--project <dir>', '--json'],
    when: 'Sau khi thêm, đổi tên hoặc chuyển asset của playable brief. Bảo đảm đổi gameplay.activeBundle là đủ để preview và mỗi brief có build config riêng.',
    outputs: ['stdout; JSON khi có --json'],
    limits: ['Chỉ kiểm tra contract tĩnh và Cocos bundle metadata; không thay thế preview/runtime smoke test.'],
    status: 'partial',
    probe: 'help',
    probeCmd: `node ${TOOLS}/gameplay-brief-validator.cjs`,
    expect: ['--project', '--json'],
  },
  {
    id: 'verify.all',
    group: 'verify',
    title: 'Cổng xác minh bắt buộc: TS, config, asset binding, meta, bundle size (trừ preview-only rõ ràng)',
    npm: 'npm run ai:verify',
    cmd: `node ${TOOLS}/headless-verifier.cjs --json`,
    args: [],
    optional: ['--json', '--skip-build-size'],
    when: 'BẮT BUỘC sau mọi lần sửa code hoặc port. Phải sạch trước khi kết thúc lượt.',
    outputs: ['stdout (JSON khi có --json)'],
    limits: ['`--skip-build-size` chỉ dành cho workflow preview-only đã cấm build: headless verifier không đọc `build/` nhưng vẫn chạy đủ TS, Zero-GC, config/binding, engine feature, `.meta` và importer checks. Build acceptance không được dùng mode này.'],
    status: 'ok',
    probe: 'help',
    probeCmd: `node ${TOOLS}/headless-verifier.cjs`,
    expect: ['--json', '--skip-build-size'],
  },
  {
    id: 'verify.prefab',
    group: 'verify',
  ...
  },
  {
    id: 'verify.assets',
    group: 'verify',
    title: 'Kiểm tra Cocos có THỰC SỰ import và dùng đúng asset không',
    npm: 'npm run ai:verify:assets',
    cmd: `node ${TOOLS}/verify-assets.cjs --json`,
    args: [],
    optional: ['--path <dir>', '--json', '--quiet'],
    when: 'Ngay sau khi sinh/sửa asset (.effect, .mtl, .prefab, texture, JSON config). Ngoài importer state, tool đối chiếu plain JSON với cc.JsonAsset cache mà preview thực sự nạp để bắt cache cũ dù source/config type đã đúng.',
    outputs: ['stdout (JSON khi có --json)'],
    limits: [
  ...
  },
  {
    ...
    args: [],
    optional: ['--file <html>', '--url <url>', '--all', '--seconds <n>', '--min-fps <n>', '--window-size <WxH>', '--viewport-size <WxH>', '--preview-device <name>', '--browser <path>', '--eval <js>', '--eval-before <js>', '--gesture <x1,y1,x2,y2,durationMs[,steps]>', '--gesture-gap <ms>', '--gesture-hold-before-move <ms>', '--gesture-keep-pressed', '--post-action-seconds <n>', '--json', '--no-screenshot'],
    ...
    limits: [
      ...
      '`--viewport-size WxH` sets the actual page viewport at device scale 1 independently of browser chrome; verify the returned canvasSize equals the native reference dimensions. A failed --eval/--eval-before makes the runtime gate fail.',
  ...
  },
  {
    ...
    limits: [
      ...
      'Combat dài hoặc reload scene giữa các tap có thể dùng gestureDelaysMs cùng số phần tử với gestures hoặc gesturesFromEvalBefore, mỗi delay trước gesture 0-60000 ms, tổng tối đa 180000 ms. Input vẫn là CDP touch lifecycle thật; không thay bằng gọi method heal/retry hay mutate HP.',
  ...
  },
  {
    ...
    args: [],
    optional: ['--project <dir>', '--config <file>', '--receipt <file>', '--output <dir>', '--suite <id>', '--no-refresh', '--json'],
    ...
    limits: [
      ...
      'Receipt schema v2 bind SHA-256 của registry, matrix, eval/reference và watchFiles theo contract `sha256-text-lf-v1` (text gộp CRLF->LF, binary giữ byte-exact) nên checkout CRLF/LF của cùng commit cho cùng digest; receipt v1 raw-byte báo stale và phải chạy lại. Thay đổi nội dung sau PASS làm `check` fail stale; `check` không ghi receipt. Tool không tự đánh giá pixel parity ngoài oracle/reference contract.',
    ...
    expect: ['init', 'run', 'check', '--risk', '--suite', '--no-refresh'],
  },
  {
    id: 'verify.gc',
    group: 'verify',
    title: 'Lint Zero-GC, module Component và quy tắc playable',
    npm: 'npm run ai:lint',
  ...
  // ─────────────────────────────────────────────────────────── optimize ──
  {
    ...
    group: 'optimize',
    title: 'Áp policy preset nén portable cho PNG/JPG/JPEG qua Cocos Asset DB',
    ...
    when: 'Sau khi checkout/sync shared kit, sau batch import texture, và trước build cuối để áp tools/texture-compression-policy.json hoặc fallback PlayableTransparent.',
    ...
    limits: [
      ...
      'Project có thể commit `tools/texture-compression-policy.json` với `default` và `overrides[].pathPrefix`; longest prefix thắng, nên UI và background có thể giữ preset khác nhau một cách portable.',
  ...
  {
    id: 'model.import-policy',
    group: 'optimize',
    title: 'Áp policy tối ưu importer cho mọi FBX qua Cocos Asset DB',
    npm: 'npm run ai:model:optimize',
    cmd: `node ${TOOLS}/model-import-policy.cjs --json`,
    args: [],
    optional: ['--project <dir>', '--directory <db://assets/...>', '--dry-run', '--verify', '--json'],
    when: 'Sau khi import/copy/export FBX và trước build cuối để mọi model dùng cùng Mesh Optimize/Simplify/Compress portable.',
    ...
    status: 'ok',
    probe: 'help',
    probeCmd: `node ${TOOLS}/model-import-policy.cjs --help`,
    expect: ['--verify', 'Mesh Optimize', 'Mesh Simplify', 'Mesh Compress'],
  },
  {
    id: 'model.fbx-normalize',
    group: 'optimize',
    title: 'Sửa FBX Unity bị Cocos converter từ chối mà vẫn giữ output FBX',
    npm: 'npm run ai:fbx:normalize -- --src <Unity.fbx> --out <Cocos.fbx>',
    cmd: `node ${TOOLS}/fbx-normalizer.cjs`,
    args: ['--src <Unity.fbx>', '--out <Cocos.fbx>'],
    optional: ['--mode <preserve|static>', '--preserve-anchor <boneName> (repeatable; static only)', '--blender <path>', '--dry-run', '--verify', '--json'],
    when: 'Chỉ dùng sau khi FBX gốc từ Unity làm Cocos FBX converter fail. Tool round-trip FBX qua Blender, tuyệt đối không sinh glTF/GLB.',
    outputs: ['<Cocos.fbx>', 'stdout receipt: format, mode, mesh/vertex/polygon/armature/action counts, preserved anchor representation'],
    limits: [
    ...
    status: 'ok',
    probe: 'help',
    probeCmd: `node ${TOOLS}/fbx-normalizer.cjs --help`,
    expect: ['--src', '--out', '--mode', 'preserve', 'static', 'No glTF or GLB'],
  },
  {
    id: 'audio.optimize',
    group: 'optimize',
    ...
    args: [],
    optional: ['--write', '--verify', '--format <mp3|ogg|wav|keep>', '--quality <10-100>', '--preserve-channels', '--mono', '--stereo', '--project <dir>', '--mcp-url <url>', '--doctor'],
    when: 'Sau import audio và trước build cuối. Mặc định scan toàn bộ sound, chuyển thành MP3 quality 30 và giữ nguyên mono/stereo của từng nguồn.',
  ...
  },
  {
    ...
    limits: ['Font diagnostics chỉ báo font asset đang dùng (theo build UUID hoặc scene/prefab reference) có coverage ngoài Basic Latin; số byte tiết kiệm là ước tính và vẫn cần tạo subset rồi verify glyph thực tế.'],
    status: 'ok',
    probe: 'help',
  ...
  },
  {
    id: 'game-hub.create',
    group: 'build',
    title: 'Tạo game hub review theo thứ tự các playable brief được chỉ định',
    npm: 'npm run hub:create',
    cmd: `node ${TOOLS}/game-hub.cjs create`,
    args: ['--brief <id>'],
    optional: ['--brief <id>', '--all', '--title <text>', '--subtitle <text>', '--out <dir>', '--project <dir>', '--build', '--jobs <n>', '--force', '--json'],
    when: 'Khi developer yêu cầu game hub, trang review brief tuần tự hoặc gom các playable brief cụ thể vào một UI Previous/Replay/Next.',
    outputs: ['game-hubs/<hub-id>/: index.html, hub.css, hub.js, manifest.json, games/*.html, .game-hub.json'],
    limits: ['Chỉ stage exact common_min build của brief đã chọn. Tạo hub không đồng nghĩa đã preview hoặc deploy; chỉ dùng --force sau khi kiểm tra output không thuộc generator.'],
    status: 'partial',
    probe: 'help',
    probeCmd: `node ${TOOLS}/game-hub.cjs`,
    expect: ['create', '--brief', '--build', '--out'],
  },
  {
    id: 'game-hub.verify',
    group: 'verify',
    title: 'Kiểm hash và inventory của game hub đã tạo',
    npm: 'npm run hub:verify',
    cmd: `node ${TOOLS}/game-hub.cjs verify`,
    args: ['--out <dir>'],
    optional: ['--project <dir>', '--json'],
    when: 'Sau mỗi lần tạo hoặc cập nhật game hub, trước preview/deploy.',
    outputs: ['stdout hoặc JSON: ok, output path, game count, issues'],
    limits: ['Chỉ xác minh file, manifest, inventory và SHA-256; vẫn phải mở hub qua HTTP và thao tác controls để claim browser acceptance.'],
    status: 'partial',
    probe: 'help',
    probeCmd: `node ${TOOLS}/game-hub.cjs`,
    expect: ['verify', '--out', 'manifest.json'],
  },
  {
    id: 'delivery.init',
    group: 'build',
    title: 'Khởi tạo config, GitHub Actions và hướng dẫn delivery cho project',
    npm: 'npm run delivery:init',
    cmd: `node ${TOOLS}/delivery-pipeline.cjs init`,
    args: [],
    optional: ['--project <dir>', '--config <file>', '--force', '--json'],
    when: 'Một lần sau khi tạo project mới hoặc khi project chưa có configs/playable-delivery.json.',
    outputs: ['configs/playable-delivery.json', '.github/workflows/playable-delivery.yml', 'PLAYABLE_DELIVERY.md'],
    limits: ['Không ghi đè config, workflow hoặc guide đã tùy chỉnh nếu thiếu --force. CI build cần Windows self-hosted runner có Cocos Creator.'],
    status: 'partial',
    probe: 'help',
    probeCmd: `node ${TOOLS}/delivery-pipeline.cjs`,
    expect: ['init', 'playable-delivery.json', 'workflow'],
  },
  {
    id: 'delivery.run',
    group: 'build',
    title: 'Build song song các brief, đóng gói nhiều hub và deploy bằng một lệnh',
    npm: 'npm run delivery',
    cmd: `node ${TOOLS}/delivery-pipeline.cjs run`,
    args: [],
    optional: ['--project <dir>', '--config <file>', '--jobs <n>', '--provider <none|netlify|github-pages>', '--skip-build', '--dry-run', '--json'],
    when: 'Khi cần build và giao một batch brief/game hub đã khai báo trong playable-delivery.json.',
    outputs: ['game-hubs/<hub-id>', '.delivery/site', 'URL Netlify hoặc GitHub Pages khi publish'],
    limits: ['Deploy chỉ chạy sau khi toàn bộ exact common_min artifact, hub manifest và delivery SHA-256 pass. Netlify cần NETLIFY_AUTH_TOKEN và NETLIFY_SITE_ID; GitHub Pages CI cần Pages permission.'],
    status: 'partial',
    probe: 'help',
    probeCmd: `node ${TOOLS}/delivery-pipeline.cjs`,
    expect: ['prepare', 'deploy', '--provider', '--jobs'],
  },
  {
    id: 'delivery.verify',
    group: 'verify',
    title: 'Xác minh read-only toàn bộ hub và site delivery',
    npm: 'npm run delivery:verify',
    cmd: `node ${TOOLS}/delivery-pipeline.cjs verify`,
    args: [],
    optional: ['--project <dir>', '--config <file>', '--json'],
    when: 'Sau khi prepare hoặc trước khi deploy lại site đã đóng gói.',
    outputs: ['stdout hoặc JSON: brief count, hub count, file/hash issues'],
    limits: ['Không thay thế browser smoke test hoặc kiểm tra HTTP của URL production.'],
    status: 'partial',
    probe: 'help',
    probeCmd: `node ${TOOLS}/delivery-pipeline.cjs`,
    expect: ['verify', 'SHA-256'],
  },
  {
    id: 'deploy.preview',
    group: 'build',
...
const CORE_RULES = [
  ...
  },
  {
    ...
    agentContract: {
      ...
      prohibitedFlagRouting: [
        'ai:verify -- --preview-only',
        'ai:port:core:verify -- --skip-build-size',
      ],
      previewClaim: ['preview-accepted', 'preview-runnable'],
      prohibitedClaim: ['build-accepted', 'build-runnable'],
    },
  },
  {
    id: 'canonical-paths',
  ...
  {
    id: 'cocos-component-module-boundary',
    rule: 'Cocos Creator 3.8.x chỉ cho một class vừa có `@ccclass` vừa kế thừa `cc.Component` (kể cả qua custom base) trên mỗi module TypeScript; Component export/re-export từ barrel cũng bị tính và có thể làm importer trả `_sealed=null`, preview đen. Tách mỗi concrete Component ra file riêng, shared base ra file riêng, giữ marker/barrel Component-free và import concrete module trực tiếp. `@ccclass` data/serializable không kế thừa Component được phép. `ai:lint` và `ai:verify` phải fail trước preview khi vi phạm.',
    agentContract: {
      appliesTo: ['cocos-typescript-source', 'unity-to-cocos-port'],
      maximumDecoratedComponentsPerModule: 1,
      componentIdentity: '@ccclass-and-extends-cc.Component-through-local-or-relative-base',
      counts: ['declared-component', 'exported-component', 're-exported-component'],
      allows: ['ccclass-non-component-data', 'component-free-marker', 'component-free-barrel'],
      requiredStructure: ['one-concrete-component-per-file', 'shared-base-own-file', 'direct-concrete-module-import'],
      failureEvidence: ['cocos-importer-rejection', 'null-_sealed', 'missing-component-registry', 'black-preview'],
      gates: ['ai:lint', 'ai:verify'],
    },
  },
  {
    id: 'cocos-compressed-component-id-resolution',
    rule: 'Cocos scene có thể serialize custom script Component bằng compressed class ID, nên không thấy full script UUID trong raw `.scene` không phải evidence component bị thiếu. Phải resolve attachment qua AssetDB/MCP component resolution (hoặc decode class ID) và xác nhận runtime class registration trước khi attach; không duplicate Component dựa trên UUID grep.',
    agentContract: {
      appliesTo: ['cocos-scene-component-audit', 'cocos-component-attachment'],
      serializedIdentity: ['full-script-uuid', 'compressed-class-id'],
      requiredBeforeAttach: ['assetdb-or-mcp-component-resolution-or-class-id-decode', 'runtime-class-registration'],
      prohibitedEvidence: ['full-uuid-absence-only', 'raw-scene-uuid-grep-only'],
      prohibitedAction: ['duplicate-component-attachment'],
    },
  },
  {
  ...
  {
    id: 'generator-cli-readonly-idempotence',
    rule: 'Mọi CLI generator phải parse mode trước khi đọc/ghi output: `--help` chỉ in usage, `--check`/`--verify` chỉ so byte và tuyệt đối không mutate, argument lạ/xung đột phải fail trước write. Generate mặc định phải so content và bỏ qua file đã giống để chạy lặp idempotent, không tranh lock với AssetDB/Editor hay làm stale receipt. Regression khóa cả SHA-256 và mtime trước/sau help, check, invalid mode và unchanged generate.',
  },
  {
    id: 'unity-port-source-evidence-first',
    rule: 'Chỉ khi sửa bug hoặc lệch fidelity của một port Unity→Cocos: trước khi sửa, phải mở lại bounded Unity source closure sở hữu hành vi lỗi (script/prefab/data/material/clip liên quan) và ghi chuỗi evidence -> root cause -> fix -> regression portable. Khi source evidence tồn tại, không được đoán offset/tint/timing/feature toggle hay thêm workaround để làm hình ảnh/flow có vẻ đúng. Nếu closure không truy cập được hoặc evidence còn thiếu, phải ghi explicit evidence gap/blocker và không được tuyên bố fidelity hay semantic parity.',
    agentContract: {
      appliesTo: ['unity-port-bug-fix', 'unity-port-fidelity-fix'],
      boundedSourceClosure: ['script', 'prefab', 'data', 'material', 'clip'],
      requiredRecord: ['evidence', 'root-cause', 'fix', 'regression'],
      prohibitedWhenSourceEvidenceExists: [
        'guessed-offset',
        'guessed-tint',
        'guessed-timing',
  ...
  },
  {
    ...
    agentContract: {
      ...
      migrationMustPreserve: ['font', 'color', 'outline-width', 'shadow-offset', 'shadow-blur'],
      verification: ['runtime-console-clean', 'visual-effect-visible', 'ai:lint', 'ai:verify'],
    },
  },
  {
  ...
  },
  {
    ...
    agentContract: {
      sourceClosure: ['scene-or-prefab-instance-overrides', 'source-particle-prefab', 'ui-particle-bridge-members', 'material-and-texture', 'renderer-and-camera', 'animation-or-callback-time'],
      resolveEffectiveInstance: true,
      bridgeMemberDispositions: ['driver', 'visible-emitter', 'dormant'],
      spatialAcceptance: ['source-viewport-bands-or-rois', 'risk-viewport-bands-or-rois', 'bounded-total-visible-pixels'],
    },
  },
  {
  ...
  {
    id: 'ui-layout-sliced-parity',
    rule: 'Khi port UGUI, phân biệt RectTransform/layout bounds với sprite draw bounds. Image Sliced/Tiled không áp preserveAspect: giữ authored rect, đúng Sprite.Type, CUSTOM sizeMode và spriteBorder left/bottom/right/top; refresh importer của sprite đã tồn tại qua AssetDB và giữ UUID. Horizontal/VerticalLayoutGroup phải giữ enabled, padding, spacing (kể cả âm), alignment, pivot, scale, reverse, active children và LayoutElement.ignoreLayout; không lấy anchoredPosition chưa chạy layout làm kết quả cuối. Porter hỗ trợ fixed-size bằng UnityFixedLayoutGroup; childControl/forceExpand, Grid hoặc fitter chưa map phải report high, không skip im lặng. Kiểm runtime hai aspect, đổi số child active và đo tâm slot/icon + frame containment. Nếu sửa lỗi lặp, sửa shared porter + regression + skill, không chỉ bù vị trí/scale riêng trong game.',
  },
  {
    id: 'unity-preflight',
