...
const CAPABILITIES = [
  ...
  },
  {
    id: 'port.preflight',
    group: 'port',
    title: 'Unity port preflight: compact feature brief + high routing + fresh mutation receipt',
    npm: 'npm run ai:port:preflight -- --project <UnityProjectRoot>',
    cmd: `node ${TOOLS}/unity-intel-cli.cjs preflight --json`,
    args: ['--project <UnityProjectRoot>'],
    optional: [
      '--provider <auto|static|unity-mcp>', '--bootstrap', '--intent <kind>', '--target <logical-path-or-symbol>',
      '--unity <file>', '--mcp-url <url>', '--timeout-ms <n>', '--include-vendor', '--cache-dir <dir>', '--no-cache', '--refresh-cache',
    ],
    when: 'BƯỚC DUY NHẤT đầu tiên trước khi agent đọc Unity source hoặc chạy tool có ghi output. Tự scan, phác thảo feature, định tuyến mọi high và cấp receipt theo state source.',
    outputs: ['stdout JSON <=12 KiB: decision, feature plan, all-code/count obligationIndex, routed high details, implementation/verification routes và receiptId'],
    limits: [
    ...
    expect: ['--project', '--provider', '--bootstrap', '--intent', '--target', '--cache-dir'],
  },
  {
    id: 'unity.intel.scan',
    group: 'port',
  ...
  },
  {
    ...
    args: [],
    optional: ['--file <html>', '--url <url>', '--all', '--seconds <n>', '--min-fps <n>', '--window-size <WxH>', '--browser <path>', '--json', '--no-screenshot'],
...
const CORE_RULES = [
  ...
  {
    id: 'unity-preflight',
    rule: 'Trước khi agent đọc raw Unity source hoặc chạy bất kỳ port tool có ghi output, BẮT BUỘC chạy `npm run ai:port:preflight -- --project <UnityProjectRoot>` (hoặc MCP `scanUnityProject`) và đọc `decision`, `features`, `obligationIndex`, `obligations`. Chỉ query evidence bounded khi brief yêu cầu. Hard blocker chặn implement; mọi source high còn lại phải có action + verification và phải được giải quyết hoặc chứng minh out-of-scope trước khi hoàn tất. Mutation receipt chỉ áp dụng cho Assets/package root đã khai báo; closure staging ngoài project phải có exact provenance và explicit --unity-project.',
    agentContract: {
      entrypoints: ['port.preflight'],
      mcpEntrypoint: 'scanUnityProject',
      mustConsume: ['decision', 'features', 'obligationIndex', 'obligations', 'receiptId'],
      before: ['read-unity-source', 'port-implementation'],
      evidenceQuery: 'unity.intel.query',
      sourceHighPolicy: 'disposition-required',
      completionHighPolicy: 'agent-disposition-required',
    },
  },
];
