...
const CAPABILITIES = [
  ...
  },
  {
    ...
    limits: [
      ...
      '`--dispositions` chỉ chấp nhận Unity Editor-confirmed missing GUID còn nằm trong core/adapter; file JSON phải nằm ngoài Unity project, khóa exact state fingerprint + owner/field/source SHA-256 + proof SHA-256, và receipt tự stale khi file evidence đổi. Một entry có thể gom `keys` (<=256, tổng <=512 GUID) khi các GUID dùng chung chính xác owner set/field/proof, ví dụ một bảng level serialized stale; từng GUID vẫn phải khớp live evidence riêng và brief chỉ giữ acceptedCount + acceptedKeysDigest. Missing reference chỉ thuộc excluded playable-core path được route defer nhưng vẫn giữ trong full high audit.',
      ...
      'Optional 3D feature chỉ được yêu cầu từ semantics reachable/active. Mesh đã serialize nhưng ParticleSystemRenderer không ở mesh mode, Unity `Debug.DrawLine/DrawRay` (chỉ vẽ trong Editor), `useOcclusionCulling = false` và asset ngoài playable-core không được phép bật module.',
  ...
  },
  {
    ...
    optional: ['--entry-scene <Assets/...unity>', '--scaffold-receipt <file>', '--provider <auto|static|unity-mcp>', '--bootstrap', '--dispositions <file>', '--engine-feature-replacements <file>', '--cache-dir <dir>', '--no-cache', '--refresh-cache', '--manifest <file>', '--target-scene <file.scene>', '--force', '--dry-run', '--json'],
    ...
    limits: [
      ...
      'Module Unity closure yêu cầu nhưng port tái tạo không cần subsystem (ví dụ ray/hull picking + Unity overlap oracle thay physics backend) chỉ được khai báo qua `tools/engine-feature-replacements.json` Git-track: exact module (thay trọn cả selector family), lý do và mandatory regression suite. Init gỡ module replaced khỏi preview như disabled module và ghi hash file vào manifest; core verify fail khi file đổi hash hoặc suite thiếu/không mandatory. Replacement không bao giờ là drop im lặng.',
    ...
    expect: ['--unity-project', '--cocos-project', '--provider', '--dispositions', '--engine-feature-replacements', '--cache-dir', '--no-cache', '--refresh-cache', '--target-scene', '--dry-run', '--force'],
  },
  {
    ...
    args: ['--unity-project <UnityProjectRoot>', '--cocos-project <CocosProjectRoot>'],
    optional: ['--entry-scene <Assets/...unity>', '--scaffold-receipt <file>', '--provider <static|auto|unity-mcp>', '--bootstrap', '--dispositions <file>', '--engine-feature-replacements <file>', '--cache-dir <dir>', '--no-cache', '--refresh-cache', '--manifest <file>', '--wiring <file>', '--packet <file>', '--target-scene <file.scene>', '--force', '--dry-run', '--json'],
  ...
  },
  {
    ...
    optional: [
      ...
      '--physics-backend <name>', '--force-physics-backend', '--skip-physics',
    ],
    ...
    limits: [
      ...
      'Model pipeline là FBX-only: copy FBX nguồn hoặc export Unity Mesh `.asset` trực tiếp thành FBX. Flag legacy `--convert-fbx-fallback` bị từ chối; không sinh glTF/GLB để che lỗi importer. Unity Mesh `.asset` (renderer lẫn MeshCollider) không bao giờ được copy raw vào Cocos vì không importer nào đọc YAML mesh; primitive theo tên chỉ là fallback khi export FBX thất bại, không dùng khi FBX đang chờ import.',
      '`--skip-physics` chỉ dùng khi playable tự tái tạo physics bằng logic riêng (raycast/overlap oracle, ballistic motion): porter bỏ Rigidbody/Collider/Joint/Effector/CharacterController, báo `low PHYSICS_COMPONENT_SKIPPED` từng component và không export/import collider mesh. Xung đột với `--physics-backend`/`--force-physics-backend`. Hành vi physics nguồn vẫn là nghĩa vụ phải có regression riêng, và engine feature physics của Unity closure cần disposition replaced.',
  ...
  },
  {
    ...
    optional: ['--dry-run', '--overwrite', '--report <path>', '--no-import-wait', '--no-engine-feature-repair', '--no-engine-feature-restart', '--physics-backend <name>', '--force-physics-backend', '--skip-physics'],
  ...
  // ────────────────────────────────────────────────────────────── verify ──
  {
    ...
    limits: [
      ...
      'Không suy optional 3D module bằng token scan: primitive cần primitive API hoặc ParticleSystemRenderer mesh mode đang active; Unity `Debug.DrawLine/DrawRay` chỉ vẽ trong Editor nên không bao giờ là evidence cho debug-renderer; evidence ngoài playable-core không được mở feature.',
  ...
  },
  {
    ...
    limits: [
      ...
      'Combat dài có thể dùng gestureDelaysMs cùng số phần tử với gestures, mỗi delay trước gesture 0-60000 ms, tổng tối đa 180000 ms. Input vẫn là CDP touch lifecycle thật; không thay bằng gọi method heal/retry hay mutate HP.',
