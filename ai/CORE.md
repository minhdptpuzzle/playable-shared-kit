...
| Khi nào dùng | Lệnh | Giới hạn cần biết |
| --- | --- | --- |
| BƯỚC ĐẦU TIÊN khi nhận một project Unity mới. Đọc cái này thay vì tự quét cây thư mục. | `npm run ai:port:plan -- --src <UnityAssetsFolder>` | Chỉ đọc — không ghi gì vào project Cocos. Bỏ qua thư mục thư viện bên thứ ba; danh sách đã bỏ qua nằm ở `skippedVendorDirs`. Số 0 trong inventory có thể nghĩa là "đã bỏ qua" chứ không phải "không có" — `demo` và `samples` nằm trong danh sách, nên khi port chính một package asset store phải thêm --include-vendor. |
...
- **`port.plan`** (npm run ai:port:plan -- --src <UnityAssetsFolder>)
  ...
  - Bỏ qua thư mục thư viện bên thứ ba; danh sách đã bỏ qua nằm ở `skippedVendorDirs`. Số 0 trong inventory có thể nghĩa là "đã bỏ qua" chứ không phải "không có" — `demo` và `samples` nằm trong danh sách, nên khi port chính một package asset store phải thêm --include-vendor.
