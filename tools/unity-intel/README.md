...
A reviewed source-disposition file (`--dispositions`) can waive a Unity Editor-confirmed missing GUID that is still routed into the playable core or adapter scope. Each entry binds the owner files, the owner field and proof files by SHA-256, together with the exact project state fingerprint. It may name one GUID in `key`, or a group of up to 256 GUIDs in `keys` when they share exactly the same owner set, field and proof, such as a stale serialized level table. The file may hold at most 128 entries and 512 GUIDs. Every grouped GUID is still checked on its own against live evidence. The brief records only `acceptedCount` and `acceptedKeysDigest`, so it stays within its byte budget.
