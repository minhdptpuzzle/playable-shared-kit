...
Each file under `.ai/port/evidence/*.json` uses `cc-playable-core-checkpoint-evidence` schema v2 and binds `briefId`, `stateFingerprint`, one checkpoint, observations, and SHA-256 for every current target file. Target hashes follow the portable `sha256-text-lf-v1` contract (`tools/lib/portable-content-hash.cjs`): text files hash with CRLF folded to LF, binary assets (png/jpg/mp3/fbx, or any file with a NUL in its first 8000 bytes) hash byte-exact. Evidence committed from a `core.autocrlf=true` CRLF checkout therefore still matches an LF checkout of the same commit. Compute hashes with `currentTargetHashes`, not a raw `sha256sum`. Schema v1 evidence hashed raw working-copy bytes and no longer counts; manifests written with the v1 contract block stay valid. Changing target content invalidates old evidence automatically. The regression receipt (schema v2) uses the same contract for registry, matrix, eval, reference, and watch files. `input-response`, `core-rules-state`, and `win-lose-restart` require `runtime` or `runtime-visual`; a screenshot alone cannot satisfy behavior. Gate subprocesses are fixed, sequential, bounded to ten minutes/2 MiB each, redact project paths and credential-shaped output, and stop on first failure.
