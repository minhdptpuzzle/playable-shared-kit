...
function randomForceContract(source){
 const force=source?.ForceModule;if(!force?.enabled)return null;
 if(!force.randomizePerFrame&&['x','y','z'].every(axis=>![2,3].includes(Number(force[axis]?.minMaxState))))return null;
 const unsupported=[];
 ...
 return{autoRandomSeed:!!source.autoRandomSeed,randomSeed:Number(source.randomSeed)>>>0,...(!force.randomizePerFrame?{randomized:false}:{}),
