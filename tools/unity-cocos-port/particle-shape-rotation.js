...
function rawRotation(rotation = {}) {
 ...
 const x=-(sx*cy*cz+cx*sy*sz),y=-(cx*sy*cz-sx*cy*sz),z=cx*cy*sz-sx*sy*cz,w=cx*cy*cz+sx*sy*sz;
...
function particleShapeRotation(rotation) { const r=rawRotation(rotation); for(const k of ['x','y','z']) { if(Math.abs(r[k]-Math.round(r[k]))<1e-10)r[k]=Math.round(r[k]); } return r; }
module.exports={particleShapeRotation};
