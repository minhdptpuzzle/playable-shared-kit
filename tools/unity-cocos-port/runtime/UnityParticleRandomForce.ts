...
import { UnityRandomForceKernel } from './UnityRandomForceKernel';
export interface UnityRandomForceSpec { autoRandomSeed: boolean;randomSeed: number;x:number[];y:number[];z:number[]; }
...
export function installUnityParticleRandomForce(system:ParticleSystem,spec:UnityRandomForceSpec):void {
    ...
    module.animate=function(p:any,dt:number):void {
        kernel.sample(random,p.unityForcePoolIndex);
