...
function stageBirthStateRuntime(options) {
  if (!options.dryRun) for (const name of ['UnityParticleBirthState','UnityParticleBirthStateAdapter','UnityParticleModuleScale']) {
...
function attachBirthStateRuntime(builder, reporter, options) {
  ...
  stageBirthStateRuntime(options);
  ...
  for(const {p,id} of particles) {
    ...
    builder.addComponent(p.node.__id__,classId,{source:{__id__:id},nativeScalingMode:p.unityParticleScalingMode??-1},null,`cmp-unity-birth-state-${id}`);
  }
}
module.exports={attachBirthStateRuntime,stageBirthStateRuntime};
