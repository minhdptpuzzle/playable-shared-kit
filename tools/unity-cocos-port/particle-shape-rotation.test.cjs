...
const {particleShapeEdgeRotation}=require('./particle-shape-rotation');
// Unity Edge: line on shape X, emission along shape +Y. Cocos Box volume emits along -Z, so under the edge basis
// Cocos -Z must land on Unity's rotated +Y and Cocos +X on Unity's rotated +X (both mirrored in Z).
for(const f of fixtures)test('Unity edge basis '+f.euler.join(','),()=>{const q=cocosQuat(particleShapeEdgeRotation({x:f.euler[0],y:f.euler[1],z:f.euler[2]}));for(const [v,axis] of [[[0,0,-1],1],[[1,0,0],0]]){const got=rotate(v,q),want=f.axes[axis].map((a,j)=>j===2?-a:a);for(let j=0;j<3;j++)assert.ok(Math.abs(got[j]-want[j])<2e-6,JSON.stringify({got,want}));}});
