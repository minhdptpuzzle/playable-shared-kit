...
class HlslAstTranspiler {
  constructor(source, options = {}) {
    ...
    /**
     * TRUE chỉ khi thân shader gốc thực sự được dịch sang GLSL.
     * Hiện tại `_generateFragmentShader()` phát ra TEMPLATE theo shading model
     * chứ không dịch thuật toán gốc, nên giá trị luôn là false.
     * Người gọi PHẢI dựa vào cờ này để chấm severity — đừng báo "transpiled".
     */
    this.bodyTranspiled = false;

    this._parse();
  }

  /**
   * Thông tin để AI agent hoàn tất phần tool không làm được.
   * - unusedUniforms: uniform đã khai báo trong UBO nhưng thân template không dùng
   *   (đây chính là các tham số thuật toán bị mất).
   * - glslDraft: bản nháp GLSL dịch từ HLSL gốc bằng `transpileHlslToGlsl()`.
   */
  getManualPortNotes(generatedEffectText = '') {
    const unusedUniforms = [];
    for (const prop of this.properties) {
      const name = prop.glslName || prop.name;
      if (!name) continue;
      if (prop.kind === 'texture' || prop.kind === 'cubemap') continue;
      // Đếm số lần xuất hiện trong phần thân đã sinh; alias khai báo 1 lần là "chỉ khai báo, không dùng".
      const occurrences = (String(generatedEffectText).match(new RegExp(`\\b${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'g')) || []).length;
      if (occurrences <= 2) unusedUniforms.push(name);
    }

    let glslDraft = '';
    try {
      const raw = (this.rawPrograms || []).join('\n\n');
      if (raw.trim()) glslDraft = this.transpileHlslToGlsl(raw);
    } catch (error) {
      glslDraft = `// [transpileHlslToGlsl] thất bại: ${error.message}`;
    }

    return { unusedUniforms, glslDraft, bodyTranspiled: this.bodyTranspiled };
  }
  ...
  _parseProperties() {
    ...
    for (const rawLine of lines) {
      ...
      // Type có thể chứa ngoặc lồng: Range(0.0, 10.0), Vector, Color, 2D, Float, Int.
      // Regex cũ dùng `[^\)]+` nên dừng ở dấu `)` đầu tiên và KHÔNG khớp được
      // `_X ("Y", Range(0,1)) = 0` -> mọi property kiểu Range bị bỏ im lặng
      // (RadialGlow mất _GlowIntensity/_Radius/_Thickness/_Sharpness).
  ...
  generateCocosEffect(effectName = 'CustomPortedEffect') {
    ...
    const effectBody = `${techniquesYaml}
...
`;
    ...
    const notes = this.getManualPortNotes(effectBody);
    const header = this.bodyTranspiled
      ? `// Auto-generated by Unity HLSL to Cocos Creator 3.8.8 Transpiler
...
//
// =====================================================================
//  SHADER_NEEDS_MANUAL_PORT - THAN SHADER CHUA DUOC DICH
// =====================================================================
//  DA sinh dung: technique, render state, properties, UBO std140, sampler.
//  CHUA dich:    thuat toan ben trong frag()/vert() - phan duoi day la
//                TEMPLATE '${this.shadingModel}', KHONG phai logic goc cua Unity.
//${notes.unusedUniforms.length ? `\n//  Uniform khai bao nhung template khong dung (= tham so thuat toan bi mat):
//    ${notes.unusedUniforms.join(', ')}\n//` : ''}
//  Ban nhap GLSL dich tu HLSL goc nam o cuoi file (khoi TODO-AGENT).
//  Nhiem vu cua agent: viet lai frag()/vert() tu ban nhap do, roi xoa
//  khoi header nay.
// =====================================================================
`;

    return `${header}
${effectBody}${this.bodyTranspiled ? '' : this._renderManualPortAppendix(notes)}`;
  }

  /** Phụ lục cuối file: bản nháp GLSL để agent dịch nốt trong đúng một lượt đọc. */
  _renderManualPortAppendix(notes) {
    const MAX_LINES = 160;
    const draftLines = String(notes.glslDraft || '').split('\n');
    const truncated = draftLines.length > MAX_LINES;
    const body = draftLines.slice(0, MAX_LINES)
      .map((line) => `//   ${line}`)
      .join('\n');

    return `
// ---------------------------------------------------------------------
// TODO-AGENT: HLSL goc da dich so bo sang GLSL (chua kiem chung).
// Dung phan nay de viet lai frag()/vert() o tren cho dung thuat toan goc.
// ---------------------------------------------------------------------
${body}
${truncated ? `//   ... (cat bot ${draftLines.length - MAX_LINES} dong - doc file .shader goc de xem day du)\n` : ''}// --------------------------- END TODO-AGENT --------------------------
`;
  }
