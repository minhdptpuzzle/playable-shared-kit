...
test('all resolved AOE particle bugs route to shared code/tests and a source-specific acceptance gate',()=>{
  const result=audit(source,root);assert.deepEqual(result.errors,[]);assert.equal(result.count,source.fixes.length);assert.ok(result.count>=26);
  ...
  assert.ok(source.fixes.some(f=>f.id==='fbx-model-mesh-basis'&&f.sharedPaths.includes('tools/unity-cocos-port/runtime/UnityModelMeshBasis.ts')));
});
