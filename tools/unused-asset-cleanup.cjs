...
function log(message) {
  console.log(`[unused-asset-cleanup] ${message}`);
}
...
function detectBundleDirectories(projectRoot, assetsRoot) {
  const bundles = [];
  for (const metaFile of walk(assetsRoot).filter((file) => file.endsWith('.meta'))) {
    ...
    if (json?.userData?.isBundle !== true) continue;
    bundles.push({
      directory: toPosix(path.relative(projectRoot, ownerFile)),
      bundleName: json.userData.bundleName || path.basename(ownerFile),
    });
  }
...
function detectDynamicResourceRoots(projectRoot, assetFiles) {
  const roots = new Set();
  const loadPattern =
    /\b(?:resources|bundle|assetManager)\s*\.\s*(?:load|loadDir|loadAny)\s*\(\s*(['"`])([^'"`]+)\1/g;
  const literalPattern = /(['"`])([^'"`\r\n]*)\1/g;
  const constPattern =
    /\b(?:const|let|var|readonly)\s+([A-Za-z_$][\w$]*)\s*(?::[^=]+)?=\s*(['"`])([^'"`\r\n$]*)\2/g;
  ...
  function addBaseName(key, asset) {
    const normalized = String(key || '').toLowerCase();
    if (!normalized) return;
    if (!assetByBaseName.has(normalized)) assetByBaseName.set(normalized, []);
    assetByBaseName.get(normalized).push(asset);
  }

  for (const asset of assetFiles) {
    const extension = path.posix.extname(asset);
    const withoutExtension = extension ? asset.slice(0, -extension.length) : asset;
    addBaseName(path.posix.basename(withoutExtension), asset);
  ...
  function addBaseNameMatchingAssets(name) {
    for (const candidate of assetByBaseName.get(String(name).toLowerCase()) || []) {
      roots.add(candidate);
    }
  }

  // `${DIR}sound_click` with `const DIR = 'sound/'` must resolve to `sound/sound_click`,
  // otherwise every template-literal asset path reads as unreferenced.
  function expandTemplate(literal, constants) {
    let resolved = true;
    const expanded = literal.replace(interpolationPattern, (whole, expression) => {
      const value = constants.get(expression.trim());
      if (value === undefined) {
        resolved = false;
        return ' ';
      }
  ...
  for (const relativeFile of assetFiles) {
    ...
    for (const match of content.matchAll(constPattern)) constants.set(match[1], match[3]);

    for (const match of content.matchAll(loadPattern)) {
      const { expanded, resolved } = expandTemplate(match[2], constants);
      if (resolved) addDirectoryMatchingAssets(expanded);
    }
    for (const match of content.matchAll(literalPattern)) {
      const literal = match[2];
      if (!literal) continue;
      const { expanded, resolved } = expandTemplate(literal, constants);
      if (resolved && (expanded.includes('/') || expanded.includes('\\'))) {
        addExactMatchingAssets(expanded);
      }
      // Unresolved interpolation still leaves static fragments such as `sound_click`,
      // which are matched by asset base name rather than by full path.
      for (const fragment of expanded.split(/[ /\\]/)) {
        if (fragment.length >= 4 && /^[\w.-]+$/.test(fragment)) {
          addBaseNameMatchingAssets(fragment);
        }
      }
    }
  }
...
function createAudit(projectRoot, options) {
  ...
  // Cocos ships every file inside an asset bundle (including `resources`) so that it can
  // be loaded by path at runtime. Static UUID analysis can never prove those unused.
  const bundleDirectories = detectBundleDirectories(projectRoot, assetsRoot);
  const bundleAssets = new Set(
    assetFiles.filter((asset) =>
      bundleDirectories.some((bundle) => asset.startsWith(`${bundle.directory}/`)),
    ),
  );
  ...
  const runtimeRoots = [...new Set([
    ...staticRoots,
    ...(options.includeBundles ? [] : bundleAssets),
  ])];
  const runtimeReachable = closure(runtimeRoots, dependencies);
  const staticReachable = options.includeBundles
    ? runtimeReachable
    : closure(staticRoots, dependencies);
  const bundleProtectedAssets = [...bundleAssets].filter(
...
function report(audit, asJson) {
  ...
  if (result.counts.bundleCount > 0) {
    const names = result.bundles.map((item) => `${item.bundleName} (${item.directory})`);
    log(`Asset bundles: ${names.join(', ')}`);
    log(
      audit.includeBundles
        ? `Bundle assets audited (--include-bundles): ${result.counts.bundleAssetCount}`
        : `Bundle assets kept as dynamic-load roots: ${result.counts.bundleAssetCount} ` +
          `(${result.counts.bundleProtectedAssetCount} kept only by this rule; ` +
          're-run with --include-bundles to audit them)',
    );
  }
